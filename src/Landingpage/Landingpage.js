...
const Landingpage = () => {
  ...
  const getdat = async () => {
    ...
    let response = await axios.get("https:// https://dollerhouse111.onrender.comprofit/alltotal-profit", {
      method: "GET",
      body: bodyContent,
      headers: headersList
    });
  ...
  const GetUserId = async (wallet_address) => {
    try {
      const response = await axios.get(
        ` https://dollerhouse111.onrender.comuser/get-user?wallet_id=${wallet_address}`
      );
